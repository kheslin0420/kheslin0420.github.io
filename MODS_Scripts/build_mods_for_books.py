...
directory = input('Enter full path to the directory where you wish for the MODS files to be stored. \nExample: /Users/name/Desktop/mods4project\n')
    ...
    for row in csv_reader:
        ...
        for i in recID:
            ...
            root2 = ET.Element(qname2,
                               ...
                               nsmap=nsmap)
            for child in mods_xml:
                root2.append(child)


        xmlString = ET.tostring(root2, encoding='utf-8', pretty_print=True)

        with open(directory + '/pitt_{}_MODS.xml'.format(row['identifier']), 'wb') as newfile:
            newfile.write(xmlString)

print("Created MODS. Moving on to inserting CSV Data....")

list_of_files = glob.glob(directory + '/*.xml') #direct script to directory containing files we just created
for file in list_of_files:
    ...
    r = xmlObject.getroot() #get the root of that object
    ...
    class Date_obj(Dates): #then create a subclass of Class Dates. This Subclass will create a date object we will used
        def __init__(self, date_list): #pass the same arguments as the parent class, plus any unique attributes. In this case
            #we do not have any new attributes to add
            self.date_list = date_list
            super().__init__(date_list)

        def get_date_obj(self): #create a function that will take the date list supplied in the class above and
            ...
            if len(self.date_list) == 3: # handles dates in YYYY, MM, DD format
                dt_obj = datetime.datetime(int(self.date_list[0]), int(self.date_list[1]), int(self.date_list[2]))
                return dt_obj
    ...
    class Display_Date(Date_obj): # create a new sublass that inherits from Date_Object sublass, so that we can create
        # a display date using the date_object created above
        def __init__(self, date_list, dt_obj): #here we do have a new attribute. We are passing the date list AND the date
    ...
    #directory_to_write_to = os.path.join('/Users/kaylaheslin/Desktop/grammy_grant2', os.path.basename(file))

    xmlString = ET.tostring(r, encoding='utf-8')
    with open(file, 'wb') as nf:
        nf.write(xmlString)

print('Data inserted and MODS are complete!')