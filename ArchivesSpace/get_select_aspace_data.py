...
path = '/path/to/where/to/save/csv'
baseURL = '' #use institutions baseURL for Aspace
user_name = '' #aspace username
password = '' #aspace api password
repo = input("Enter repo number:")
record_type = 'archival_objects' #change according to record type 
auth = requests.post('{}/users/{}/login?password={}'.format(baseURL, user_name, password)).json()
...
heirarchy = input('Is their heirarchy to this collection? (ie are there series/subseries/record groups?) y/n?:').lower()

##Nested archival objects
with open('{}/{}.csv'.format(path, csv_path), mode=csv_mode) as csv_file:
    ...
    if heirarchy == 'y':
        ...
        seriesORsuberies_level_endpoint = '/repositories/{}'.format(repo) + '/archival_objects/' + str(
            seriesORsubseries_num)
        ...
        if arg == 'true':
            ...
            for item in ao:  # looking at the items within the subseries or series
                ...
                for i in child_list:
                    ...
                    if "digital_object" in instance_type_list:
                        do = requests.get(
                            baseURL + i['instances'][instance_type_list.index('digital_object')]['digital_object'][
                                'ref'], headers=headers).json()
            ...
            children_endpoint = '/repositories/{}'.format(repo) + '/archival_objects/' + str(
                seriesORsubseries_num) + '/children'
            ao = requests.get(baseURL + children_endpoint,
                              headers=headers).json()  # request the json for each item (subseries or children) under the series
            for item in ao:  # looking at the items within the subseries
                ...
                if "digital_object" in instance_type_list:
                    do = requests.get(
                        baseURL + item['instances'][instance_type_list.index('digital_object')]['digital_object'][
                            'ref'], headers=headers).json()
                    identifier = do['digital_object_id']
                    title = item['title']
        ...
        for child in collection_tree['children']:
            ...
            if "digital_object" in instance_type_list:
                do = requests.get(
                    baseURL + ao['instances'][instance_type_list.index('digital_object')]['digital_object']['ref'],
                    headers=headers).json()
                identifier = do['digital_object_id']
                title = do['title']
